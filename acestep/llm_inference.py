...
def _monitor_mem_frag(device, label: str = "") -> None:
    """Monitor XPU memory fragmentation and alert when severe.

    XPU lacks CUDA's expandable segments and complex OOM retry chain,
    so memory fragmentation accumulates as KV cache grows. This function
    tracks inactive_split_bytes to detect fragmentation trends early.
    """
    ...
    try:
        ...
        frag_ratio = inactive_split_bytes / (allocated + 1)
        if inactive_split_bytes > 0:
            frag_mb = inactive_split_bytes / 1024**2
            alloc_mb = allocated / 1024**2
            if frag_ratio > 0.3:
                print(f"  🧩 [{label}] 碎片警告：inactive_split={inactive_split} 块, "
                      f"{frag_mb:.0f}MB（占已分配 {frag_ratio:.1%}），"
                      f"已分配={alloc_mb:.0f}MB")
                torch.xpu.empty_cache()
            elif frag_ratio > 0.1:
                print(f"  📊 [{label}] 碎片状态：{frag_mb:.0f}MB inactive ({frag_ratio:.1%})")
    except Exception:
        pass
...
class LLMHandler:
            ...
            for step in tqdm(range(max_new_tokens), desc="LLM Constrained Decoding", unit="token", disable=self.disable_tqdm):
                ...
                if (step + 1) % 50 == 0 and step < max_new_tokens - 1:
                    ...
                    if len(_llm_step_times) >= 3:
                        ...
                        if len(_llm_step_times) > 20:
                            _llm_step_times.pop(0)
                        median_t = sorted(_llm_step_times)[len(_llm_step_times)//2]
                        if step_elapsed > median_t * 2.0:
                            print(f"  🌡️  LLM 降频（{step_elapsed:.2f}s > 基线{median_t:.2f}s × 2），额外冷却 10 秒...")
                            time.sleep(10)
                    _llm_step_t0 = time.time()
                    gc.collect()
    ...
    def _load_model_context(self):
        ...
        try:
            ...
            # 在 model.to("cpu") 之前先清理缓存，减少 XPU 内存压力
            # XPU 缺乏 CUDA 的复杂重试链，直接 to("cpu") 可能因显存碎片触发 OOM
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            elif hasattr(torch, 'xpu') and torch.xpu.is_available():
            ...
            try:
                ...
                logger.warning(
                    f"[load_model_context] model.to('cpu') failed ({type(exc).__name__}: {exc}), "
                    "likely GPU state corruption. Continuing cleanup..."
                )
