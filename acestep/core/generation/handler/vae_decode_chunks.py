...
class VaeDecodeChunksMixin:
    ...
    def _tiled_decode_inner(self, latents, chunk_size, overlap, offload_wav_to_cpu):
        ...
        # XPU 没有 OutOfMemoryError 异常，需要同时捕获 RuntimeError（XPU OOM 表现为
        # RuntimeError + Level Zero 错误码）和 CUDA 的专用异常。
    ...
    def _tiled_decode_gpu(self, latents, stride, overlap, num_steps):
        ...
        for i in tqdm(range(num_steps), desc="Decoding audio chunks", disable=self.disable_tqdm):
            ...
            # ---- GPU 冷却：每 50 步休息 5 秒 ----
            if (i + 1) % 50 == 0 and i < num_steps - 1:
                step_elapsed = time.time() - _decode_step_t0
                if len(_decode_step_times) >= 3:
                    _decode_step_times.append(step_elapsed)
                    if len(_decode_step_times) > 20:
                        _decode_step_times.pop(0)
                    median_t = sorted(_decode_step_times)[len(_decode_step_times)//2]
                    if step_elapsed > median_t * 2.0:
                        print(f"  🌡️  解码降频（{step_elapsed:.2f}s > 基线{median_t:.2f}s × 2），额外冷却 10 秒...")
                        time.sleep(10)
                _decode_step_t0 = time.time()
                gc.collect()
                if hasattr(torch, 'xpu'):
                    torch.xpu.empty_cache()
                print(f"  💤 解码冷却：第 {i+1}/{num_steps} 块，休息 5 秒...")
                time.sleep(5)
